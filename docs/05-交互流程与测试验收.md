...
5. 写入剪贴板前按实际写入文本标记为软件内部复制；粘贴后恢复原剪贴板内容时也会标记，避免被监听器误认为外部复制。
...
1. 用户在外部或应用内文本输入框聚焦时按快速候选热键，默认 `Ctrl+Alt+Space`。
2. 主窗口优先通过 `RegisterHotKey` 接收热键；若注册失败，低层键盘钩子作为兜底。
3. `TextInputContextService` 获取当前输入上下文，优先使用真实 caret，其次 UIA 选区/焦点，最后使用鼠标位置。
4. 物理屏幕坐标按当前显示器 origin 和 DPI 转换为 WPF DIP 坐标。
5. `QuickPasteWindowPlacement` 将弹窗放在光标附近；靠右/靠底时自动避让。
6. 弹窗中输入关键字搜索标题、内容和标签；上下键切换，Enter 或双击插入，Esc 关闭。
7. 长标题、长内容和长标签可通过鼠标悬停查看完整 ToolTip。

## 剪贴板监听流程

1. `ClipboardWatcher` 监听 `WM_CLIPBOARDUPDATE`。
2. 如果剪贴板无文本或为空白，忽略。
3. 如果内容匹配软件内部复制标记，忽略；不同外部文本不会提前消费内部标记。
...
- 当前左侧激活具体分类时，点击“新建片段”会默认选中该分类。
- 当前为“全部”“未分类”或“回收站”时，新建片段不预选分类。
- 新建/编辑片段弹窗使用搜索框 + 树形勾选选择分类，并用 chip 展示已选分类。
...
当前自动化测试重点覆盖非 UI 逻辑，包括文本处理、数据库、导入导出、快速候选搜索、候选位置计算、DPI 坐标转换和剪贴板过滤；WPF 视觉和系统级热键/剪贴板交互仍以启动冒烟与手工验收为主。
...
- 编辑片段标签并保存后，卡片标签立即刷新。
- 新建分类弹窗打开后文本框自动聚焦。
- 左侧“未分类”按钮只显示没有任何分类关系的片段。
- 新建片段默认带入当前激活分类；弹窗分类选择器支持搜索和树形勾选。
- 分类树新增/删除后保持展开状态。
- 同名分类拖拽合并不崩溃。
- 剪贴板监听不会提示本软件内部复制出去的内容，包括变量替换后的复制/粘贴内容和粘贴后恢复的原剪贴板内容。
- 热键在 Windows 10/11 虚拟桌面中从当前桌面唤出窗口。
- 快速候选热键能在文本输入框焦点下弹出候选窗口，位置靠近输入光标；右侧/底部边界不越界。
- 快速候选候选项上下键切换高亮明显，长标题/长内容/长标签悬停能查看完整内容。
- 帮助菜单“查看日志”能打开日志查看器，支持刷新、复制日志和打开日志目录。
- 回收站恢复、永久删除、清空符合预期。
